...
- Documentation is organized by product area under `docs/`. Each product area gets its own directory (for example `docs/power-pages/`, `docs/canvas-apps/`, `docs/powerapps-component-framework/`) and the directory-level `index.md` acts as the landing page for that area.
- At the moment, `docs/power-pages/index.md` is the only populated page; the other product-area `index.md` files are placeholders. When adding new content, keep it under the appropriate product-area directory instead of creating a flat list of top-level docs.
...
- Follow the existing documentation style from `CONTRIBUTING.md` and `docs/power-pages/index.md`: use `##`/`###` headings without skipping levels, tables for structured comparisons, blockquotes for important responsibility callouts, and checklist items for review gates.
